...
- 时间：ISO-8601。
- 错误：业务异常返回 `500` + 文本消息（本期未统一错误体，`SHOULD` 后续规范为 `{code,message}`）。
...
| GET/POST/PUT/DELETE | `/control-lines` | 控制线 CRUD |
| GET | `/export` | 整体导出 `SchoolDataset`（供引擎拉取）；含 `highSchools/juniorSchools/quotaSeats/controlLine/scoreLines` + **`scoreSegments`（2025/2026 一分一段表，用于志愿模拟换算区排名）** |
...
| GET | `/students/{id}/applications` | 某考生志愿（单考生有界，不分页） |
| GET | `/students/junior/{juniorSchoolId}` | 某初中校考生列表（分页，见 4.6） |
| POST | `/applications/batch` | 整页保存某考生志愿（按 batch） |
...
- `AdmissionResult` 字段含 `schoolRank`（校内排名）：仅 `batch=QUOTA` 且 `ADMITTED` 时有效，表示「同初中校竞争同一高中校额」的名次；统招与未录取为 `null`。`GET /results`、`/results/student/{id}` 均返回该字段。
