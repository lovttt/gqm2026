...
- **志愿生成器（Generator）**：输入通勤上限、高考出口梯队、跨区投放、综合素质评价、梯度权重、偏好权重等参数，按校额到校 / 统招 / 贯通三档展示志愿方案（**贯通批次为演示占位、不参与录取**），并对通勤超限、梯队不符、贯通门槛、梯度权重合计等做实时校验与联动对比。
- **录取模拟**：校额到校（控制线 + 校内排名）、统招平行志愿兜底，支持一键全量模拟并输出统计（**仅两批次录取；贯通批次为前端演示占位，不入录取**）。
- **多角色视图**：考生端、管理端、学校端。
...
```powershell
cd backend
# 方式 A：一键构建并启动全部 5 个服务（含约 10120 名考生种子），约 2 分钟
...
登录后在考生端点击「**志愿生成器**」进入 `/generator`，填写参数后点击「生成」，即可查看校额/统招/贯通三档方案（**贯通为演示占位**）、校验问题与权重联动对比。管理员无关联考生时可在页面顶部选择考生。
